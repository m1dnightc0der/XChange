...
public class DeribitAdapters {
    ...
    public static String adaptInstrumentName(Instrument instrument) {
        if (instrument instanceof FuturesContract) {
            ...
            return adaptInstrumentName((CurrencyPair) instrument);
        }
    ...
    public static Ticker adaptTicker(DeribitTicker deribitTicker) {
        return new Ticker.Builder()
                ...
                .high(deribitTicker.getStats().getHigh())
                .low(deribitTicker.getStats().getLow())
                ...
                .markPrice(deribitTicker.getMarkPrice())
                .settlementPrice(deribitTicker.getSettlementPrice())
                .openInterest(deribitTicker.getOpenInterest())
                .askIv(deribitTicker.getAskIv())
                .bidIv(deribitTicker.getBidIv())
                .markIv(deribitTicker.getMarkIv())
                .deliveryPrice(deribitTicker.getDeliveryPrice())
                .estimatedDeliveryPrice(deribitTicker.getEstimatedDeliveryPrice())
                .currentFunding(deribitTicker.getCurrentFunding())
                .funding8h(deribitTicker.getFunding8h())
                .indexPrice(deribitTicker.getIndexPrice())
                .instrumentName(deribitTicker.getInstrumentName())
                .interestRate(deribitTicker.getInterestRate())
                .underlyingIndex(deribitTicker.getUnderlyingIndex())
                .underlyingPrice(deribitTicker.getUnderlyingPrice())
                .vega(deribitTicker.getGreeks()==null ? null : deribitTicker.getGreeks().getVega())
                .theta(deribitTicker.getGreeks()==null ? null : deribitTicker.getGreeks().getTheta())
                .rho(deribitTicker.getGreeks()==null ? null : deribitTicker.getGreeks().getRho())
                .gamma(deribitTicker.getGreeks()==null ? null : deribitTicker.getGreeks().getGamma())
                .delta(deribitTicker.getGreeks()==null ? null :deribitTicker.getGreeks().getDelta())
                .build();
    }
    ...
    public static OrderBook adaptOrderBook(DeribitStreamingOrderBook deribitOrderBook) {
        Instrument instrument = adaptInstrument(deribitOrderBook.getInstrumentName());
        List<LimitOrder> bids =
                adaptOrdersList(deribitOrderBook.getBids(), Order.OrderType.BID, instrument);
        List<LimitOrder> asks =
    ...
    public static Trades adaptTrades(List<DeribitTrade> deribitTrades, Instrument instrument) {
        ...
        return new Trades(
                deribitTrades.stream()
                        .map(trade -> adaptTrade(trade, instrument))
                        .collect(Collectors.toList()));
    }


    public static List<Ticker> adaptTickers(List<DeribitTicker> deribitTickers, Instrument instrument) {
        return deribitTickers.stream()
                .map(ticker -> adaptTicker(ticker))
                .collect(Collectors.toList());
    ...
    public static OpenPosition adapt(Position p) {
        return new OpenPosition.Builder()
                ...
                .type(p.getSize().compareTo(BigDecimal.ZERO)<0 ? OpenPosition.Type.SHORT : OpenPosition.Type.LONG)
                .size(p.getSize())
                .unRealisedPnl(p.getFloatingProfitLoss())
    ...
    public static FuturesContract adaptFuturesContract(DeribitInstrument instrument) {
        ...
        if (!PERPETUAL.equalsIgnoreCase(instrument.getSettlementPeriod())) {
            expireDate = formatDate(instrument.getExpirationTimestamp());
    ...
    public static Instrument adaptInstrument(String instrumentName) {
        ...
        if (parts.length == 1) {
            ...
            if (parts[0].contains("_")) {
                ...
                currencyPair =
                        new CurrencyPair(subParts[0], subParts[1]);
