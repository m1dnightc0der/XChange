...
public interface TradeService extends BaseService {
  ...
  static OrderQueryParamInstrument[] toOrderQueryParamsInstrument(
      Instrument instrument, String... orderIds) {
    OrderQueryParamInstrument[] res = new OrderQueryParamInstrument[orderIds.length];
    for (int i = 0; i < orderIds.length; i++) {
      String orderId = orderIds[i];
      res[i] = new DefaultQueryOrderParamInstrument(instrument, orderId);
    }
    return res;
  }
